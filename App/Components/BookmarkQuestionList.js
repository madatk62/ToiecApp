...
export default class WrongAnswerList extends Component {
    ...
    fetchData = async () => {
        const data = await requestGET(`${HOST}/tests/viewListBookmarkQuestion/${DeviceInfo.getDeviceId()}`)
        this.setState({ data: data.data })
    }
    renderItem = ({ item, index }) => {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate("BookmarkQuestion", { id: item.exercise_id, name: item.exercise_description })} >
                <View style={{
                    flex: 1,
                    flexDirection: 'row',
    ...
    renderBody = () => {
        ...
        else return <Empty theme={this.state.darkMode} />
    }
    render() {
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: this.state.darkMode === false ? '#EEEEEE' : '#212121' }}>
                <View style={{
                    ...
                    <Text style={styles.title}>Câu yêu thích</Text>
